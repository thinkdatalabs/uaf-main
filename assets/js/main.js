...
(function() {
  ...
  function updateCountDown(countDownItem) {
    const targetTime = new Date(countDownItem.getAttribute('data-count')).getTime();
    const now = new Date().getTime();
    const timeleft = targetTime - now;

    if (timeleft <= 0) {
      countDownItem.innerHTML = "<h3>We're Live!</h3>";
      return;
    }
  ...
  document.querySelectorAll('.countdown').forEach(function(countDownItem) {
    ...
    let interval = setInterval(function() {
      if (new Date(countDownItem.getAttribute('data-count')).getTime() - new Date().getTime() <= 0) {
